...
class User:
    def __init__(self, name="anonymous", info=None):
        ...
        Will create a new binding between a name and a User object in users table.
        """
    ...
    def info(self, value):
        """
        Setter function for info attribute.
        Raises a TypeError if input is not a dictionary with the right volumePref, password, and token key formats.
        """
        if value is None:
    ...
    def upload(self):
        """
        Uploads a user to the users table. Raises an AssertionError if name is taken
        """
    ...
    def created(name):
        """
        Checks if a user has been created and stored in the database.
        """
    ...
    def validate(name):
        """
        Returns True if a user is in the database,
    ...
    def get_all_users():
        """
        Gets all users in the database, along with their info.
        """
        sqlite3.register_converter("user", User.convert_user)
        with sqlite3.connect(database, detect_types=sqlite3.PARSE_DECLTYPES) as c:
            c.execute('''CREATE TABLE IF NOT EXISTS users (name text UNIQUE, u user);''')
    ...
    def update(self):
        """
        Updates the user stored in the database to match the current Python object.
        """
        sqlite3.register_adapter(User, User.adapt_user)
        with sqlite3.connect(database, detect_types=sqlite3.PARSE_DECLTYPES) as c:
            c.execute('''CREATE TABLE IF NOT EXISTS users (name text UNIQUE, u user);''')
